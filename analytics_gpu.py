...
def navi_and_up(nc, num):
    ...
    (status,FreqGfx)=subprocess.getstatusoutput("upp -p /sys/class/drm/card"+ num +"/device/pp_table get smc_pptable/FreqTableGfx/"+ str(dpm_str[-1]))
    ...
    dpm_str = dpm_str.replace('ERROR: Decoded data does not contain any value, you probably wanna look deeper into data;', '').replace('\nERROR:Incorrectvariablepath:smc_pptable/FreqTableUclk','').replace(' ','').replace('\nERROR:Incorrectvariablepath:smc_pptable/FreqTableUclk','').split(',')
    init_data[nc][num]['FreqTableUclk_dpm_min'] = int(dpm_str[1])
    init_data[nc][num]['FreqTableUclk_dpm_max'] = int(dpm_str[-1])
    (status,FreqTableUclk)=subprocess.getstatusoutput("upp -p /sys/class/drm/card"+ num +"/device/pp_table get smc_pptable/FreqTableUclk/"+ str(dpm_str[-1]))
